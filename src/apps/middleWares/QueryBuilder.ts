    ...
    sort() {
        ...
        if (this.query?.sortBy || this.query?.sortOrder) {
            ...
            if (sortOrder !== 'asc' && sortOrder !== 'desc') {
                throw new AppError(400, "Sort Order of the this.Query only recieve 'asc' or 'desc' . Nothing else ")
            }
            sort = this.query?.sortOrder === 'desc' ? `-${this.query?.sortBy}` : `${this.query?.sortBy}`
        }
        this.QueryModel = this.QueryModel.find().sort(sort)
        return this
    }
